...
class ActivityET extends ETServer<Activity> implements ESServer<Activity>, EAServer<Activity>{
    constructor(){
        super(Activity);
    }    

    toValueLabel(item: Activity): ESOption {
    ...
    async findSelectObject(query: ESQuery): Promise<ETResult<Activity & ESOption>> {
                const pageIndex = parseInt(query.pageIndex)  || 0;
                const pageSize = parseInt(query.pageSize) || 25;
                ...
                // Add relations
                qb.leftJoinAndSelect("activity.activityStatus", "activityStatus")
                .leftJoinAndSelect("activity.timesheetType", "timesheetType")
                .leftJoinAndSelect("activity.subContractor", "subContractor")
                .leftJoinAndSelect("activity.contractService", "contractService")
...
const activityET = new ActivityET();


export default activityET;